...
from .styles import PHASE_COLORS, get_style
...
def _draw_paper_world(ax, trace: SimulationTrace, frame: int, *, show_map: bool) -> None:
    ...
    for index, agent_id in enumerate(trace.agent_ids):
        ...
        ax.add_patch(
            Circle(point, 0.22 * radius, facecolor=style.agent_core, edgecolor="none", zorder=9)
        )
        if agent_id in contacts or agent_id in pushers:
            ax.add_patch(
                Circle(
                    point,
                    1.45 * radius,
                    facecolor="none",
                    edgecolor=PHASE_COLORS[
                        "TRANSPORT" if agent_id in pushers else "CONTACT_READY"
                    ],
                    linewidth=1.25,
                    zorder=10,
                )
...
def _directional_progress(trace: SimulationTrace):
    ...
    for cargo_id in trace.cargo_ids:
        ...
        target = trace.target_distance[cargo_id]
        if np.isfinite(target):
            ax.axhline(target, color="#b91c1c", linestyle="--", linewidth=1.1, label="target L")
...
def _coverage_and_gap(trace: SimulationTrace):
    ...
    gap = trace.max_uncovered_gap[cargo_id]
    if np.any(np.isfinite(gap)):
        axes[1].plot(trace.times, gap, color="#b45309", linewidth=1.8)
        gap_label = "Max gap [m]"
    else:
        axes[1].text(
            0.5,
            0.5,
            "Metric not recorded by Claude v2 core",
        ...
        gap_label = "Max gap [m] (unavailable)"
    _paper_axes(axes[1], trace, None, "Time [s]", gap_label)
    for ax in axes:
        _shade_phases(ax, trace)
    fig.tight_layout()
    return fig
...
def _safety(trace: SimulationTrace):
    ...
    if np.isfinite(d_min):
        axes[0].axhline(d_min, color="#991b1b", linestyle="--", linewidth=1.0, label="d_min")
...
def _control_effort(trace: SimulationTrace):
    ...
    if np.any(np.isfinite(force)):
        ...
        _unavailable(axes[0], "Net force not stored in replay.npz")
    _paper_axes(axes[0], trace, "Control effort and contact", None, "Net force [N]")
    torque = trace.net_torque[cargo_id]
    if np.any(np.isfinite(torque)):
        axes[1].plot(trace.times, torque, color="#7c3aed", linewidth=1.6)
    else:
        _unavailable(axes[1], "Net torque not stored in replay.npz")
...
def _unavailable(ax, message: str) -> None:
    ax.text(
        0.5,
        0.5,
        message,
