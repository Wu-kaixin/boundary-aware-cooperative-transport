...
class DBACTController:
        ...
        for i, agent in enumerate(agents):
            ...
            if self.params.task_mode == "coverage":
                ...
                if method in {"arm", "b0"}:
                    ...
                    u_nom = u_nom + self._observation_transport_bias(agent, observations)
                    mode = "dbact_cage"
                else:
    ...
    def _observation_transport_bias(
        ...
        if not self.params.enable_transport_bias or not observations:
            return np.zeros(2, dtype=float)
        vd = np.asarray(self.params.task_velocity, dtype=float).reshape(2)
        if float(np.linalg.norm(vd)) < 1e-9:
            return np.zeros(2, dtype=float)
        threshold = self.params.cage_offset + self.params.transport_contact_margin
        for obs in observations:
            if float(np.linalg.norm(agent.position - obs.point)) <= threshold:
    ...
    def _oracle_command(
        ...
        if self.params.enable_transport_bias:
            for cargo in cargoes:
