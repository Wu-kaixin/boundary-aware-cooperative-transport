...
Ported from the CODEX branch: :func:`maximum_uncovered_boundary_arc` and
:func:`operational_enclosure_certificate`. The gap they close is that *mean*
coverage cannot distinguish the two configurations that matter here. Fifteen of
sixteen robots evenly spread and fifteen piled on one side both report a high
:func:`strict_boundary_coverage`, and only the second has an open arc wide enough
for the object to leave through. ``minimum_facing_cage_clearance`` predicts that
failure mode before a run; this measures it during one. Both are audit-only and
neither is reachable from the controller.
"""
...
def maximum_uncovered_boundary_arc(covered: np.ndarray, perimeter: float) -> dict[str, float | int]:
    """Conservative sampled upper bound on the longest cyclic uncovered arc.

    Cyclic, for the same reason ``enclosure_gate.largest_gap`` is: an open arc
    straddling sample 0 is one arc, and reporting it as two understates exactly
    the piled-on-one-side configuration this measure exists to catch.

    The ``(longest + 1) * resolution`` term is the conservative direction. The
    uncovered run is bounded by covered samples on both ends, and the true
    boundary between covered and uncovered lies somewhere inside each of those two
    sampling intervals, so the continuous arc can exceed ``longest * resolution``
    by up to two half intervals. Reporting ``longest * resolution`` would be
    optimistic by exactly the quantity being bounded.
    """
    ...
    if np.all(mask):
        ...
        longest, upper = count, float(perimeter)
    else:
...
def operational_enclosure_certificate(
    ...
    *,
    ...
    """Truth-audit certificate for operational boundary enclosure at one frame.

    Deliberately not a configuration-space escape proof, and therefore
    ``formal_caging`` is a constant ``False`` here exactly as it is in
    :mod:`dbact.guarantees`. What is certified is measurable: boundary occupancy
    by robots whose centres are outside, no uncovered arc wider than the declared
    bound, pairwise separation, offset-curve compatibility, and an engaged quorum.

    Robots inside the object are excluded from the covering set before anything is
    measured. That is the same correction :func:`strict_boundary_coverage` makes,
    and it matters more here: a robot that has passed through the boundary is
    simultaneously close to samples on both sides of it, so including it would
    close the very arc whose width is the quantity of interest.
    """
