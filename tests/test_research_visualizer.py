...
def test_demo_theme_preserves_claude_v1_visual_language(trace):
    visualizer = ResearchVisualizer(trace, view_mode="demo")
    assert visualizer.style.world_face == "#d7dadd"
    assert visualizer.style.cargo_face == "#ff7f0e"
    assert visualizer.style.agent_face == "#7ec8ee"
    assert visualizer.voronoi_collection.get_visible()
    assert all(patch.get_visible() for patch in visualizer.safety_rings)
    assert all(patch.get_visible() for patch in visualizer.communication_rings)
    assert all(patch.get_facecolor()[:3] == pytest.approx((0.494, 0.784, 0.933), abs=0.01)
               for patch in visualizer.agent_patches)
    visualizer.close()
