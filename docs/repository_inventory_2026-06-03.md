# Repository Inventory - 2026-06-03
...
This inventory lists the source, config, test, documentation, and platform files that should be visible to GitHub. Generated outputs such as `runs/`, `.pytest_cache/`, `__pycache__/`, virtual environments, experiment CSVs, and plot artifacts are intentionally excluded by `.gitignore` and are not part of the committed source inventory.
...
| Root DBACT source | 12 | Core algorithm, local sensing, density, CVT, CBF, metrics, types. |
...
| Root scripts | 3 | Batch scenario runner, mock MAS pipeline, tree utility. |
| Root tests | 6 | DBACT unit, smoke, MAS adapter, and coverage-mode tests. |
| Root docs | 10 | Architecture, algorithm, roadmap, stage notes, daily health, inventory. |
| Vendored MAS platform | 100 | Platform ignore rules, apps, configs, docs, tests, source, and NatNet client files. |

Total Git-visible source/inventory files after this update: 169.
...
The inventory was prepared while `main` had local changes that are intended to be committed with this documentation update:

```text
M  README.md
M  scripts/make_repo_tree.py
M  scripts/run_all_scenarios.py
M  src/dbact/boundary_density.py
M  src/dbact/controller.py
M  src/dbact/local_cbf_qp.py
M  src/dbact_sim/environment.py
M  src/dbact_sim/run_sim.py
M  src/dbact_sim/visualization.py
A  configs/sim/decentralized_cvt_coverage.yaml
A  configs/sim/paper_like_irregular_moving_cargo.yaml
A  docs/daily_health_2026-06-03.md
A  docs/repository_inventory_2026-06-03.md
A  tests/test_decentralized_coverage.py
```
